...
class Manage extends Component{
	constructor(){
        ...
        componentWillMount()
        {
            axios.get('/Farm/manage').then(response => { 
               this.setState({farm:response.data.data,
	...
	confirm(id){
		axios.get('/Delete/Farms/'+id).then(response => {
                    ...
                    <thead>
                   	...
                   	<tbody>
						...
						<tr key={i} className="odd gradeX">
							 <td><img style={{height: 100,width: 100}} src={'/FarmHouseImages/'+v.submitted_by}/></td> 
							
							<td>{v.fname}</td>
							<td>{v.address}</td>
							 
							   <td className="center">{v.from_Date}</td>
							   <td className="center">{v.to_Date}</td>
							   ...
							   <td>
							<Button.Group>
    						<Button  onClick={this.confirm.bind(this,v.farmid)} negative>Delete</Button>    		
